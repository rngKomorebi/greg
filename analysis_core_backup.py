...
def _annotate_sigma_labels(
    ...
    placed: list = []
    box_style = dict(
        boxstyle="round,pad=0.25", fc="white", ec="black", alpha=0.85
    )
...
def plot_neighbour_scales(
    ...
    ax_scale.scatter(xmid, scales, marker="x", zorder=5)
    ax_scale.axhline(
        mean_scale,
        linestyle="--",
        linewidth=1,
...
def plot_allpair_scales(
    ...
    colors = [
        plt.get_cmap("tab20")(k) for k in np.linspace(0, 1, max(n_groups, 1))
    ]
    ...
    ax.set_title("nm/px scale across all peak pairs")
    if scale_ylim is not None:
    ...
    fig.subplots_adjust(left=0.10, right=0.72, bottom=0.12, top=0.95)
...
def plot_scale_vs_separation(
    ...
    ax.scatter(dist_px, scales, marker="x")
    ax.axhline(
        mean_scale,
        linestyle="--",
        linewidth=1,
    ...
    ax.set_xlabel("Pixel separation between peaks")
    ax.set_ylabel("Scale (nm/pixel)")
    ax.set_title("nm/px scale vs peak separation")
    if scale_ylim is not None:
        ax.set_ylim(*scale_ylim)
    ax.legend()
...
def plot_ideal_spectrum(
    ...
    for i, ln in enumerate(ax.lines):
        ln.set_label("Data" if i == 0 else "_nolegend_")
    ...
    in_range = (ideal_px >= sensor_range[0]) & (ideal_px <= sensor_range[1])
    ...
    for i, (x0, amp) in enumerate(zip(ideal_px, amps)):
        ...
        ax.plot(
            ...
            linewidth=3,
            color=ideal_color,
            label="Ideal spectrum, NIST" if i == 0 else "_nolegend_",
        )
...
def plot_linearity(
    ...
    ax_top.scatter(px, wl, label="Data")
    ax_top.plot(px_dense, a * px_dense + b, label=f"Fit  a={a:.3f} nm/px")
    ax_top.set_ylabel("Wavelength (nm)")
    ax_top.legend(loc="best", frameon=True)
    ax_top.set_title("Detector linearity")

    ax_bot.axhline(0.0, linewidth=1)
    ax_bot.scatter(px, residuals)
...
def plot_gaussian_fit(
    ...
    ax.plot(x_plot, y, marker="o", linewidth=1.5, markersize=4, label="data")
    ax.plot(x_plot, y_fit, linewidth=2.5, label="fit")
    ...
    if use_wl:
        for i in range(0, len(popt) - 1, 3):
            popt_ann[i + 1] = float(_px_to_wl([popt[i + 1]], ref_pixels)[0])
    ...
    fig.subplots_adjust(left=0.10, right=0.88, bottom=0.14, top=0.95)
...
def run_neon_analysis(
    ...
    sigma_px           : Gaussian σ in pixels for the ideal-spectrum overlay (plot D)
...
pkl_file = r"D:\LinoSPAD2\Data\B7d\2026.02.23\300 200 mm\300 mm\results\sensor_population\0000073443-0000073443_rates.pickle"
detected_pixels = [32, 59, 128, 170, 224]
ref_wavelengths_nm = [640.225, 638.224]
ref_pixels = [32, 59]

run_neon_analysis(pkl_file, detected_pixels, ref_wavelengths_nm, ref_pixels)
